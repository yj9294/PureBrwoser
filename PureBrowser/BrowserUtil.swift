...
class BrowserUtil: NSObject {
    ...
    var isLoading: Bool {
        webView.estimatedProgress > 0 && webView.estimatedProgress < 1.0
    }
...
extension BrowserUtil {
    ...
    func goForword() {
        if item.webView.canGoForward {
            item.webView.goForward()
        }
    }
    
    func loadUrl(_ url: String, from vc: HomeVC) {
    ...
    func addedWebView(from vc: HomeVC) {
        if webView.url != nil  {
            vc.view.insertSubview(webView, belowSubview: vc.settingView)
        }
    }
